...
class Vehicle:
  __COLOR_VARIANTS = ['blue', 'red', 'green', 'black', 'white']  # Допустимые цвета для окрашивания транспорта

  def __init__(self, owner: str, model: str, color: str, engine_power: int):
    self.owner = owner                                          # владелец транспорта (владелец может меняться)
    self.__model = model                                        # модель транспорта (НЕЛЬЗЯ менять название модели)
    self.__engine_power = engine_power                          # мощность двигателя (нельзя менять)
    self.__color = color                                        # цвета (нельзя менять)
